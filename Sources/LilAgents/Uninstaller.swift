...
enum Uninstaller {
    ...
    private static let sudoersPath = "/etc/sudoers.d/lilagents"
    /// Pre-rename path of the same rule. Removed here too — a machine that
    /// upgraded but never re-enabled stay-awake still has it, and leaving a
    /// root-owned NOPASSWD rule behind on uninstall is a security problem.
    private static let legacySudoersPath = "/etc/sudoers.d/agentdeck"
    ...
    static func performUninstall() {
        ...
        do {
            if FileManager.default.fileExists(atPath: LilAgents.supportDir.path) {
                try FileManager.default.removeItem(at: LilAgents.supportDir)
            }
            // Normally already gone — launch migrates it into the directory
            // above — but a migration that failed must not leave the old
            // directory (token included) behind on a full uninstall.
            if FileManager.default.fileExists(atPath: LilAgents.legacySupportDir.path) {
                try FileManager.default.removeItem(at: LilAgents.legacySupportDir)
            }
        } catch {
        ...
        // Preferences (it's not under LilAgents.supportDir) and the prompt
        // never fires.
    ...
    /// NOPASSWD rule StayAwakeController installed — under either its current
    /// or its pre-rename path — mirroring its install mechanism (a single
    /// AppleScript "with administrator privileges" prompt covering all of it).
    /// Idempotent: does nothing (and prompts nothing) if none is present.
    /// Returns a non-nil message on failure.
    private static func revertStayAwake() -> String? {
        let existingSudoersPaths = [sudoersPath, legacySudoersPath].filter {
            FileManager.default.fileExists(atPath: $0)
        }
        let isAwake = readSleepDisabled()
        guard !existingSudoersPaths.isEmpty || isAwake else { return nil }

        var commands: [String] = []
        if isAwake { commands.append("\(pmsetPath) -a disablesleep 0") }
        if !existingSudoersPaths.isEmpty {
            commands.append("/bin/rm -f \(existingSudoersPaths.joined(separator: " "))")
        }
