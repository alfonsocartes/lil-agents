...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Take over a pre-rename support directory BEFORE anything reads or
        // creates the current one — the token below is the file that has to
        // survive, since every already-running CLI session's forwarder reads
        // it on each event. Best-effort and never fatal (see the function).
        ...
        // regardless of the LILAGENTS_NO_INSTALL path below — the listener
        // requires it on every request.
        let token = LilAgents.loadOrCreateToken()
        ...
        // `LilAgentsApp` (status icon + session dropdown); the legacy
        ...
        // Silent on the LILAGENTS_NO_INSTALL smoke-test path for the same
    ...
    private func applySessionTracking(enabled: Bool) {
        ...
        hookMutationTask = Task.detached(priority: .utility) {
            ...
            do {
                ...
                NSLog("lil agents hook \(action) failed: \(error)")
            }
        }
    }
    ...
    private func registerOverlayHotkey() {
        ...
        NSLog("lil agents hotkey ⌥⌘J registered: \(registered)")
    }
