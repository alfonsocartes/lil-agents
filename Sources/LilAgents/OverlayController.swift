...
final class OverlayController {
        ...
        activeSpaceObserver = NSWorkspace.shared.notificationCenter.addObserver(
            ...
            MainActor.assumeIsolated {
                ...
                if let panel = self.panel {
                    NSLog("lil agents overlay space change: visible=\(panel.isVisible) onActiveSpace=\(panel.isOnActiveSpace) frame=\(NSStringFromRect(panel.frame))")
                }
            }
        }
    }
    ...
    func show() {
        ...
        if !panel.isVisible || !OverlayPlacement.isDisplayable(panel.frame) {
            ...
            NSLog("lil agents overlay show: recreating visible=\(panel.isVisible) onActiveSpace=\(panel.isOnActiveSpace) frame=\(NSStringFromRect(panel.frame)) screens=\(NSScreen.screens.count)")
            panel.orderOut(nil)
            panel = makePanel()
            self.panel = panel
            panel.present()
        ...
        NSLog("lil agents overlay show: visible=\(panel.isVisible) onActiveSpace=\(panel.isOnActiveSpace) frame=\(NSStringFromRect(panel.frame)) screens=\(NSScreen.screens.count)")
    }
    ...
    private func makePanel() -> FloatingPanel<OverlayView> {
        // FloatingPanel's init calls setFrameAutosaveName("LilAgentsPanel"),
