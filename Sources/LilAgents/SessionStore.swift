...
final class SessionStore {
    ...
    /// idle/waiting sessions from `byID` after `LilAgents.staleAfter` (1hr)
    ...
    func pruneStale(protecting protectedIDs: Set<String> = []) {
        let cutoff = now().addingTimeInterval(-LilAgents.staleAfter)
        ...
        // can't grow unboundedly over a long-running lil agents process.
        let notifiedCutoff = now().addingTimeInterval(-LilAgents.staleAfter * 24)
