...
final class StayAwakeController {
    ...
    /// Where the same rule lived before the rename. Removed by the installer
    /// below rather than left behind: a stale root-owned NOPASSWD rule keeps
    /// granting passwordless `pmset` forever, which is a security problem and
    /// not just an untidy leftover.
    ...
    private func runPmsetDisableSleep(_ enable: Bool) async -> Bool {
        ...
        // The probe result alone isn't sufficient: `sudo -n` succeeds if ANY
        // file under /etc/sudoers.d grants the rule, and the pre-rename file
        // is functionally identical to ours. On an upgraded machine that
        // legacy file lets the probe pass forever, so `installSudoers()` (the
        // only place that removes it) would never run and the stale root-owned
        // NOPASSWD rule would never get cleaned up. Force one more install
        // pass whenever the legacy file is still present, even if sudo
        // already succeeded.
        let hasLegacyRule = FileManager.default.fileExists(atPath: Self.legacySudoersPath)
        if first.status == 0 && !hasLegacyRule { return true }
    ...
    private static func buildAdminInstallAppleScript() -> String? {
        ...
        guard username.range(of: "^[A-Za-z_][A-Za-z0-9_-]*$", options: .regularExpression) != nil else {
            NSLog("lil agents: refusing to install sudoers rule — unexpected system username \"\(username)\"")
            return nil
        }
        ...
        let steps = [
            "tmp=$(/usr/bin/mktemp /tmp/lilagents-sudoers.XXXXXX)",
            ...
            // Same admin authorization, so the pre-rename rule is retired
            // without a second password prompt. `-f` keeps the `&&` chain
            // succeeding when there is nothing to remove (the common case).
            "/bin/rm -f \(legacySudoersPath)",
        ]
