...
/// with header `X-LilAgents-Token: <per-install bearer token>`.
///
...
final class EventListener: Sendable {
    ...
    private let queue = DispatchQueue(label: "lilagents.listener")
    ...
    func stop() {
        ...
        existing.cancel()
        NSLog("lil agents listener stopped")
    }
    ...
    private func bind(generation: UInt64, attempt: Int) {
        ...
        do {
            ...
            newListener.stateUpdateHandler = { [weak self] state in
                ...
                switch state {
                case .ready:
                    NSLog("lil agents listening on 127.0.0.1:\(LilAgents.port)")
                case .failed(let err):
                    ...
                    NSLog("lil agents: ⚠️ FAILED to bind 127.0.0.1:\(LilAgents.port) — \(err). Another process may already be bound to this port and intercepting hook events; lil agents will not receive session updates until this is resolved.")
            ...
            NSLog("lil agents could not start listener: \(error)")
    ...
    private func evaluate(_ buffer: Data) -> Evaluation {
        ...
        guard let providedToken = Self.headerValue(header, name: "x-lilagents-token"),
              LilAgents.constantTimeEquals(providedToken, token)
    ...
    private func dispatch(_ body: Data) {
        guard let event = try? JSONDecoder().decode(HookEvent.self, from: body) else {
            NSLog("lil agents: undecodable event body")
            return
        }
